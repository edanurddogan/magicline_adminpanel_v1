              ...
              <q-item v-for="col in props.cols" :key="col.name">
                ...
                <q-item-section side>
                  ...
                  <!-- <q-item-section v-else-if="col.name === 'action'" class="row">
                    <div class="row">
                      <q-btn dense flat round color="primary" icon="info" @click="onView(props.row)" />
                      <q-btn dense flat round color="secondary" icon="edit" @click="onEdit(props.row)" />
                      <q-btn dense flat round color="negative" icon="delete_forever" @click="onDeleteUser(props.row)" />
                    </div>
                  </q-item-section> -->
  ...
  computed: {
    columns() {
      return [
        {
          name: "recipeNo",
          label: this.$t("products.recipeNo"),
          align: "left",
          field: (row) => row.recipeNo,
          sortable: true,
        },
        {
          name: "productname",
        ...
        {
          name: "stocktype",
          label: this.$t("products.createdAt"),
          align: "left",
  ...
  methods: {
    ...
    async getList() {
      try {
        ...
        this.rows = response?.data;
        this.$logger.log("rows production receipe", this.rows);
        this.loading = false;
      } catch (error) {
        this.$logger.log(error);
      }
