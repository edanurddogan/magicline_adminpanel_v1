    ...
    <div class="row">
      <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
        <q-card class="q-ma-xs col-lg-12 col-md-12 col-sm-12 col-xs-12">
              ...
              <q-item class="col-lg-2 col-md-2 col-sm-6 col-xs-12">
                <q-btn-toggle
                  ...
                  :options="[
                    ...
                    { value: 'warehouseTransfer', slot: 'two' },
          ...
          <q-card-section class="q-pa-none">
            <q-list class="row">
              <!--Warehouse transfer selection-->
              <q-item
                v-if="stockMovement.movement == 'warehouseTransfer'"
                class="col-lg-4 col-md-4 col-sm-6 col-xs-12"
              >
                <q-item-section>
                  <q-select
                    v-model="stockMovement.warehouseOut"
                    outlined
                    use-input
                    transition-show="scale"
                    transition-hide="scale"
                    ...
                    option-value="id"
                    option-label="name"
                    :label="$t('products.warehouseOut')"
                    :options="warehouse"
                    @update:model-value="(val) => warehouseOutSelected(val)"
                  >
                    <template v-slot:no-option>
                      <q-item>
                        <q-item-section class="text-grey">
              ...
              <q-item
                v-if="
                  !stockCard.serialNumberTracking &&
                  (stockMovement.movement == 'stockIn' ||
                    stockMovement.movement == 'warehouseTransfer')
                "
                class="col-lg-3 col-md-3 col-sm-3 col-xs-3"
              >
                <q-item-section>
                ...
                <q-item-section>
                  <q-btn
                    ...
                    @click="
                      ...
                      printSerialNo.piece = stockCard.piece;
                    "
                  />
                </q-item-section>
              </q-item>
            ...
            <q-item class="col-lg-2 col-md-2 col-sm-2 col-xs-6">
              <q-item-section>
                <q-input
                  ...
                  v-model="printBarcode.printedPiece"
                />
              </q-item-section>
            </q-item>
            <q-item class="col-lg-2 col-md-2 col-sm-2 col-xs-6">
...
export default defineComponent({
  ...
  methods: {
    ...
    async getLastSerialNo() {
      try {
        const response = await this.$api.get(
          `getLastSerialNo/${this.stockCard.id}/${this.stockMovement.piece}`
        );
        this.printSerialNo_Control = false;
        this.serialNos = response?.data;
        this.newSerialNo = response?.data[0];
        this.newSerialNoEnd = response?.data[response?.data.length - 1];
      } catch (error) {
        this.$logger.log(error);
      }
    },
    ...
    async warehouseOutSelected(warehouse) {
      this.$logger.log("warehouseOut", warehouse);
    },
    async getUnit() {
    ...
    async getSerialNos() {
      ...
      try {
        const response = await this.$api.get(
          `getSerialNosWithID/${this.stockCard.id}`
        );
        this.serialNos = response?.data;
        this.$logger.log("serial nos", this.serialNos);
        this.loadingBarcode = false;
      } catch (error) {
        this.$logger.log(error);
      }
    ...
    async stockInOutSave() {
      try {
        ...
        if (this.serialNos && this.serialNos.length) {
          this.stockMovement["stockSerialNos"] = this.serialNos;
        }
        const _response = this.$api
    ...
    async printLabel() {
      ...
      labelScriptDetail.piece = this.printBarcode.piece;

      this.$logger.log("labelScriptDetail", labelScriptDetail);
      this.$logger.log("this.printBarcode.piece", this.printBarcode.piece);
      const _response = this.$api
