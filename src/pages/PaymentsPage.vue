...
export default defineComponent({
      ...
      return [
        {
          ...
          label: "id",
          align: "left",
          field: (row) => row.chargeTransactionId,
          sortable: true,
        },
        {
          name: "region",
          ...
          align: "left",
          field: (row) => row.preauthAmount,
          sortable: true,
        },
        {
          name: "station",
          label: this.$t("station.station"),
          align: "left",
        ...
        },
        {
          ...
          align: "left",
          sortable: true,
          field: (row) => this.formatDateTR(row.startTime),
          sortable: true,
        },
        {
          name: "nameAndSurname",
        ...
        {
          name: "totalEnergy",
          label: `${this.$t("reports.usage")} ${this.$t("reports.kW")}`,
          align: "right",
          style: "padding-right: 30px;",
          field: (row) => row.totalEnergy.toFixed(2),
          sortable: true,
        },
        {
          name: "price",
  ...
  mounted() {
    document.title = `${firm.name}-${this.$t("payments")}`;
    this.init();
  ...
  methods: {
    ...
    // async getList() {
    //   try {
    //     const response = await this.$api.get("getOngoingTransactionsByAdmin");
    //     this.rows = response.data.data || [];

    //     this.$logger.log("ongoing transactions", this.rows);
    //   } catch (error) {
    //     this.$logger.error("ongoing transactions", error);
    //   }
    // },
