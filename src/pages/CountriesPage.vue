    ...
    <q-dialog v-model="deleteConfirm" persistent>
      <q-card class="q-pa-md">
        ...
        <q-card-section class="items-center">
          ...
          <h5>{{ $t("definitions.countrieDelete") }}</h5>
          <p>
            {{ $t("definitions.countryName") }} : {{ currentRow.countryName }}
          </p>
        </q-card-section>
...
import CountriesForm from "src/components/forms/CountriesForm.vue";
  ...
  computed: {
    columns() {
      return [
        {
          name: "countryName",
          label: this.$t("definitions.countryName"),
          align: "left",
          field: (row) => row.countryName,
          sortable: true,
        },
        {
          name: "action",
  ...
  methods: {
    ...
    async getList() {
      try {
        ...
        this.$logger.log("countries =>", response.data);
        this.loading = false;
      } catch (error) {
