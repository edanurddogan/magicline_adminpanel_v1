...
<template>
  <div class="q-pa-md">
    ...
    >
      ...
      <template v-slot:top="props">
        <div class="row col-12">
          ...
          >
            <q-tooltip>
              {{ $t("discountList.addDiscountInformation") }}
            </q-tooltip>
          </q-btn>
          <q-space />
          <q-btn
    ...
    >
      <q-card style="min-width: 75vw; max-height: 70vh">
        <q-card-section class="q-pa-none">
          <q-bar class="q-pa-sm">
            ...
            <q-btn dense flat icon="close" @click="onFormCompleted()">
      ...
      <q-card class="q-pa-md text-center">
        ...
        <q-card-section>
          <div class="text-h6 q-mb-sm">
            {{ $t("discountList.discount") }} {{ $t("deleteConfirm") }}
          </div>
          <div class="text-subtitle2">
            {{ $t("discountList.discountCode") }} : <b>{{ currentRow.code }}</b>
          </div>
        </q-card-section>
...
import DiscountForm from "src/components/forms/DiscountForm.vue";
...
export default defineComponent({
    ...
    columns() {
      return [
        ...
        },
        {
          ...
          field: (row) => row.discountType.name,
          align: "left",
          sortable: true,
        },
        {
          name: "stationElectricType",
          label: this.$t("station.electricType"),
          field: (row) =>
            row.stationElectricType && row.stationElectricType.name
              ? row.stationElectricType.name
              : "AC, DC",
          align: "left",
          sortable: true,
        },
        {
          ...
          field: (row) => (row.percentage ? " % " + row.percentage : ""),
          align: "left",
          sortable: true,
        },
        {
        ...
        },
        {
          ...
          field: (row) => this.formatDate(row.startDate),
          align: "left",
          sortable: true,
        },
        {
          ...
          field: (row) => this.formatDate(row.endDate),
          align: "left",
          sortable: true,
        },
        {
  ...
  methods: {
    init() {
      document.title = `${firm.name}-${this.$t("pages.discountList")}`;
      this.getList();
    },
    onAdd() {
    ...
    async getList() {
      try {
        this.loading = true;
        const response = await this.$api.get("getDiscounts");
        this.rows = response.data.data;
        this.$logger.log("Discounts fetched:", this.rows);
      } catch (error) {
        this.$logger.error("Error fetching discount list:", error);
        this.$q.notify({
          type: "negative",
    ...
    exportTable() {
      ...
      XLSX.writeFile(workbook, "DiscountList.xlsx");
    },
    ...
    async deleteRow() {
      try {
        ...
        this.$logger.log("del", this.currentRow);
        await this.$api.put(`deleteDiscount/${this.currentRow.id}`);
