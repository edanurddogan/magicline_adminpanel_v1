...
import InvoiceForm from "src/components/forms/InvoiceForm.vue";
...
export default defineComponent({
  ...
  computed: {
    columns() {
      return [
        {
          name: "invoiceNo",
          label: this.$t("finance.invoiceNo"),
          align: "left",
          field: (row) => row.invoiceNo,
          sortable: true,
        },
        {
          name: "firm",
        ...
        {
          name: "invoiceDate",
          label: this.$t("finance.invoiceDate"),
          align: "left",
          field: (row) => row.invoiceDate,
          sortable: true,
        },
        {
          name: "invoiceProcessType",
          label: this.$t("finance.invoiceProcessType"),
          align: "left",
          field: (row) => row.invoiceTransactionType.name,
          sortable: true,
        },
        {
          name: "invoiceType",
          label: this.$t("finance.invoiceType"),
          align: "left",
          field: (row) => row.invoiceProcessType.name,
          sortable: true,
        },
        {
          name: "processType",
          label: this.$t("finance.processType"),
          align: "left",
          field: (row) => row.invoiceTransactionType.name,
          sortable: true,
        },
        ...
        {
          name: "vatTotal",
          label: this.$t("order.vattotal"),
          align: "left",
          field: (row) => row.vatTotal,
          sortable: true,
        },
        {
          name: "grandTotal",
  ...
  async mounted() {
    document.title = `${firm.name}-${this.$t("finance.invoices")}`;
    this.init();
  },
  methods: {
    init() {
    ...
    exportTable() {
      ...
      XLSX.writeFile(workbook, "InvoicesList.xlsx");
    },
    ...
    async getList() {
      try {
        const response = await this.$api.get("getInvoiceList/99/1");
        this.rows = response?.data;
        this.$logger.log("getInvoiceList", this.rows);
        this.loading = false;
      } catch (error) {
        this.$logger.log(error);
      }
