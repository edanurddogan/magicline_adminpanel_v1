...
<template>
  <div class="q-pa-md">
      ...
      <template #body-cell-action="props">
        <q-td :props="props">
          <q-btn
            ...
            color="purple"
            icon="settings"
            :disable="props.row.isActive === false"
            @click="onAdvancedSettings(props.row)"
          >
    ...
    <!-- Advanced Settings Form Dialog -->
    <q-dialog
      v-model="formVisibleSettings"
      persistent
      :maximized="maximizedToggle"
      transition-show="slide-up"
      transition-hide="slide-down"
    >
      <q-card style="min-width: 55vw; max-height: 90vh">
        ...
        <q-card-section class="scroll q-pa-sm">
          <station-advanced-settings-form
            class="q-pa-sm"
            v-model="currentRow"
            :mode="currentMode"
...
import StationAdvancedSettingsForm from "src/components/forms/StationAdvancedSettingsForm.vue";

export default defineComponent({
  components: {
    StationForm,
    StationAdvancedSettingsForm,
  },
  data() {
    return {
      rows: [],
    ...
    columns() {
      return [
        ...
        },
        {
          ...
          label: this.$t("station.privateArea"),
          field: (row) => (row.isPrivateLocation ? this.$t("confirm") : "-"),
          align: "left",
          sortable: true,
        },
        {
  ...
  methods: {
    init() {
      document.title = `${firm.name}-${this.$t("station.stations")}`;
      this.getList();
    },
    onAdvancedSettings(row) {
      this.maximizedToggle = false;
      this.currentRow = row;
      this.currentMode = "advancedSettings";
      this.formVisibleSettings = true;
    },
    onAdd() {
      this.maximizedToggle = false;
      this.currentRow = {};
    ...
    async deleteRow() {
      try {
        ...
        this.$q.notify({
          type: "positive",
          message: `${this.$t("station.station")} ${this.$t("deleted")}`,
        });
      } catch (error) {
    ...
    exportTable() {
      ...
      // İşlem sütunlarını hariç tut
      const exportColumns = this.columns.filter((col) => col.label !== "İşlem");
      ...
      XLSX.writeFile(workbook, "StationsList.xlsx");
    },
    clickMenu(row) {
    ...
    async getList() {
      try {
        ...
        //this.$logger.log("getStations:", this.rows);
        this.loading = false;
      } catch (error) {
