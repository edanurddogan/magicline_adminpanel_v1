...
<template>
  <div class="q-pa-md">
      ...
      <template v-slot:top="props">
        <div class="row col-12">
          <div class="col-lg-2 col-md-6 col-sm-12 col-xs-12 q-table__title">
            {{ $t("technicalService.technicalServiceStatuses") }}
          </div>
          <q-input
          ...
          >
            <q-tooltip transition-show="flip-right" transition-hide="flip-left">
              {{ $t("technicalService.technicalServiceStatusesAdd") }}
            </q-tooltip>
          </q-btn>
          <q-btn
            flat
    ...
    >
      <q-card style="min-width: 50vw; max-height: 90vh">
        ...
        <q-card-section style="max-height: 70vh" class="scroll q-pa-sm">
          <technical-service-statuses-form
            class="q-pa-sm"
            v-model="currentRow"
            :mode="currentMode"
    ...
    <!-- TechnicalServiceStatusesPage Delete -->
    <q-dialog v-model="deleteConfirm" persistent>
      <q-card class="q-pa-md">
        <q-card-section class="items-center"> </q-card-section>
        <q-card-section class="items-center">
          <q-avatar icon="delete" color="negative" text-color="white" />
          <h5>{{ $t("technicalService.technicalServiceStatusesDelete") }}</h5>
          <p>{{ $t("technicalService.process") }} : {{ currentRow.process }}</p>
          <p>{{ $t("technicalService.status") }} : {{ currentRow.status }}</p>
          <p>{{ $t("technicalService.color") }} : {{ currentRow.color }}</p>
        </q-card-section>
...
import TechnicalServiceStatusesForm from "src/components/forms/TechnicalServiceStatusesForm.vue";
import exportFromJSON from "export-from-json";
import { Platform } from "quasar";

export default defineComponent({
  components: {
    TechnicalServiceStatusesForm,
  },
  computed: {
    columns() {
      return [
        {
          name: "process",
          label: this.$t("technicalService.process"),
          align: "left",
          field: (row) => row.name,
          sortable: true,
        },
        {
          name: "situation",
          required: true,
          label: this.$t("technicalService.situation"),
          align: "left",
        ...
        {
          name: "color",
          label: this.$t("technicalService.color"),
          field: (row) => row.color,
          align: "center",
          sortable: true,
        },
  ...
  methods: {
    ...
    onView(TechnicalServiceStatuses) {
      this.maximizedToggle = false;
      this.currentRow = TechnicalServiceStatuses;
      this.currentMode = "details";
      this.formVisible = true;
    },
    onEdit(TechnicalServiceStatuses) {
      this.maximizedToggle = false;
      this.currentRow = TechnicalServiceStatuses;
      this.currentMode = "edit";
      this.formVisible = true;
    },
    onDeleteUser(TechnicalServiceStatuses) {
      this.currentRow = TechnicalServiceStatuses;
    ...
    exportTable() {
      ...
      const fileName = "TechnicalServiceStatuses";
    ...
    async userDelete() {
      const response = await this.$api.delete(
        "technicalServiceStatuses/" + this.currentRow.id
      );
      this.getList();
    ...
    async getList() {
      try {
        const response = await this.$api.get("technicalServiceStatuses");
        this.rows = response?.data;

        this.$logger.log("technicalServiceStatuses =>", response.data);
        this.loading = false;
      } catch (error) {
