    ...
    <q-dialog v-model="deleteConfirm" persistent>
      <q-card class="q-pa-md">
        ...
        <q-card-section class="items-center">
          ...
          <h5>{{ $t("definitions.citieDelete") }}</h5>
          <p>{{ $t("definitions.countryId") }} : {{ currentRow.countryId }}</p>
          <p>{{ $t("definitions.cityName") }} : {{ currentRow.cityName }}</p>
        </q-card-section>
...
import CitiesForm from "src/components/forms/CitiesForm.vue";
...
export default defineComponent({
  ...
  computed: {
    columns() {
      return [
        {
          name: "country",
          label: this.$t("definitions.country"),
          align: "left",
          field: (row) => row.country.countryName,
          sortable: true,
        },
        {
          name: "cityName",
          label: this.$t("definitions.cityName"),
          align: "left",
          field: (row) => row.cityName,
          sortable: true,
        },
        {
          name: "action",
  ...
  methods: {
    ...
    async userDelete() {
      const response = await this.$api.delete("cities/" + this.currentRow.id);
      this.getList();
    ...
    async getList() {
      try {
        const response = await this.$api.get("citiesList");
        this.rows = response?.data;

        this.$logger.log("cities =>", response.data);
        this.loading = false;
      } catch (error) {
