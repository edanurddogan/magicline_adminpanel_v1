        ...
        <template #body-cell-action="props">
          <q-td :props="props">
            ...
            <q-btn
              ...
              color="negative"
              icon="delete_forever"
              @click="deletionRequest(props.row)"
            />
          </q-td>
        </template>
      ...
      <q-card class="q-pa-md">
        <q-card-section class="items-center row">
          <q-avatar
            ...
            text-color="white"
          />&nbsp; &nbsp; &nbsp; &nbsp;
          <h5>{{ $t("authorization.employeegroupdeleteconfirm") }}</h5>
        </q-card-section>
        <q-card-section class="items-center row">
          <p>{{ $t("authorization.employeegroup") }} : {{ currentRow.name }}</p>
        </q-card-section>
        ...
        <q-card-actions align="right">
          ...
          <q-btn
            ...
            :label="$t('delete')"
            color="negative"
            v-on:click="onDelete(currentRow)"
          />
        </q-card-actions>
      </q-card>
    </q-dialog>
...
export default defineComponent({
  ...
  methods: {
    async init() {
      try {
        const response = await this.$api.get("employeeGroups");
        this.$logger.log("employeeGroups", response?.data);
        this.rows = response?.data.filter((item) => item.name !== "Admin");
        this.loading = false;
      } catch (error) {}
    },
    onSubmit() {},
    ...
    async addFormModel() {
      const _response = this.$api
        .post("employeeGroups", JSON.stringify(this.newModel))
        .then((response) => {
    ...
    async editFormModel() {
      const response = this.$api
        .put("employeeGroups", JSON.stringify(this.newModel))
        .then((_response) => {
    ...
    async onEdit(row) {
      this.newModel = row;
      this.formMode = "edit";
      this.selectedbrand.id = this.newModel.brandId;
      this.$logger.log("selectedbrand", this.selectedbrand);
    },
    async deletionRequest(row) {
      try {
        const response = await this.$api
          .get("getCountEmployeeintheGroup/" + row.id)
          .then((resp) => {
            if (parseInt(resp.data) == 0) {
              ...
              this.$q.notify({
                ...
                textColor: "white",
                message: this.$t(
                  "authorization.employeegroupcannotdelete"
                ).replace("_count_", resp.data),
    ...
    async getList() {
      const response = this.$api.get("employeeGroups").then((data) => {
        ...
        this.rows = data.data.filter((item) => item.name !== "Admin");
