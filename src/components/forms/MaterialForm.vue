<template>
  ...
  <!-- Save Confirm Dialog -->
  <q-dialog v-model="saveConfirmVisible" persistent>
    <q-card class="q-pa-md">
      <q-card-section class="items-center row">
        <q-avatar icon="warning" color="amber" text-color="white" />
        <h5 class="q-ml-md">{{ $t("warning") }}</h5>
      </q-card-section>
      <q-card-section class="items-center row">
        {{ warningMessage }}
      </q-card-section>
      ...
      <q-card-actions align="right">
        ...
        <q-btn
          ...
          :label="$t('save')"
          color="negative"
          v-on:click="(saveConfirm = true), addFormModel()"
        />
      </q-card-actions>
    </q-card>
  </q-dialog>
...
export default defineComponent({
  ...
  methods: {
    ...
    save() {
      var requiredList = [];
      if (this.formModel.stockName) {
        this.formModel.stockName = this.formModel.stockName;
      } else {
        requiredList.push("Stok Adı");
      }
      if (this.formModel.unit) {
        this.formModel.unit = this.formModel.unit;
      } else {
        requiredList.push("Birim");
      }
      if (this.formModel.currency) {
        this.formModel.currencyId = this.formModel.currency.id;
      } else {
        requiredList.push("Para Birimi");
      }
      if (this.formModel.listPrice) {
        this.formModel.listPrice = this.formModel.listPrice;
      } else {
        requiredList.push("Liste Fiyatı");
      }
      if (this.formModel.vatRate) {
        this.formModel.vatRate = this.formModel.vatRate;
      } else {
        requiredList.push("KDV Oranı");
      }
      ...
      const warningFields = [
        { key: "stockType", idKey: "stockTypeId", label: "Stok Tipi" },
        { key: "category", idKey: "categoryId", label: "Kategori" },
      ...
      warningFields.forEach((field) => {
        if (this.formModel[field.key]) {
          ...
          warningList.push(field.label);
        }
      });

      if (warningList.length > 0 && this.currentMode === "add") {
        this.warningMessage =
          warningList.join(", ") + " Seçilmemiş. Kayıt edilsin mi?";
      ...
      try {
        if (this.saveConfirm) {
          const _response = this.$api
            .post("materials", JSON.stringify(this.formModel))
            .then((response) => {
              ...
              this.$q.notify({
                ...
                type: "info",
                color: "info",
                message: "Yedek Parça Bilgileri Kaydedildi", //this.$t("success"),
    ...
    async editFormModel() {
      try {
        const response = this.$api
          .put("materials", JSON.stringify(this.formModel))
          .then((_response) => {
        ...
        this.$logger.error("Error editFormModel:", error);
      }
    },
