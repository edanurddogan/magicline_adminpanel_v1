          ...
          <q-item class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
            <q-item-section>
              <q-select
                ...
                @filter="filterFunc"
                @update:model-value="updateBrand($event)"
              >
...
export default defineComponent({
  ...
  methods: {
    async init() {
      this.getBrands();
      try {
        const response = await this.$api.get("modellist");
        this.$logger.log("categories", response?.data);
    ...
    async addFormModel() {
      const _response = this.$api
        .post("models", JSON.stringify(this.newModel))
        .then((response) => {
    ...
    async editFormModel() {
      const response = this.$api
        .put("models", JSON.stringify(this.newModel))
        .then((_response) => {
    ...
    async onDelete(row) {
      try {
        ...
        const response = await this.$api.delete("models/" + row.id);
        this.getList();
    ...
    async getList() {
      const response = this.$api.get("modellist").then((data) => {
    ...
    filterFunc(val, update) {
      if (val === "") {
        update(() => {
          this.modelFiltered = this.modelFull;
        });
        return;
      }

      update(() => {
        this.modelFiltered = this.modelFull.filter(
    ...
    updateBrand(event) {
      if (this.selectedbrand) this.newModel.brandId = this.newModel.brand.id;
      this.$logger.log(this.newModel);
    },
  },
});
